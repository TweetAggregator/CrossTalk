...
class TweetStreamer(query: List[(TweetQuery, ActorRef)]) extends Actor {
  ...
  val waitToExplore = getConfInt("tweetStreamer.waitToExplore", "TweetStreamer: key waitToExplore not defined in conf.")
  ...
  def receive = {
    ...
    case Wait => /* Nothing to do, a stream continue forever. This is just for TweetSearcher. */
